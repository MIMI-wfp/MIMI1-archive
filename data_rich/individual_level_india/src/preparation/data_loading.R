...
sf_use_s2(FALSE)
india_adm2 <- st_join(other_shape, st_centroid(india_adm2), st_contains ) 
india_adm2 <- india_adm2 %>% 
  # filter(d_name == shapeName)%>% 
  select(!c('d_name', "shapeType"))%>% rename(ADM2_NAME = shapeName) %>% 

  # mutate(ADM2_NAME = ifelse(ADM2_NAME == "Aravali", "Sabar Kantha", ADM2_NAME))
...
unique(india11_census$`State name`)
