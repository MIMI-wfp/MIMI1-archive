...
table1(~ ADM1_NAME + AGE_YEAR +WEIGHT + HEIGHT + BMI | SEX, data = adults)
...
# scatter plot of difference in inadequacy per admin 2
  ...
  va_scat <- vita_women_ml %>% 
    select(inadequate_percent, ADM2_NAME,s_name) %>% 
    inner_join(vita_target %>% rename(ADM2_NAME =d_name) %>%  select("ADM2_NAME", inad_diff), by = "ADM2_NAME") %>% 
  ...
  fo_scat <- fo_women_ml %>% 
    select(inadequate_percent, ADM2_NAME,s_name) %>% 
    inner_join(folate_target %>% rename(ADM2_NAME =d_name) %>% select(ADM2_NAME, inad_diff), by = "ADM2_NAME")
  
  #######
  ir_scat <- ir_women_ml %>% 
    select(inadequate_percent, ADM2_NAME,s_name) %>% 
    inner_join(iron_target %>% rename(ADM2_NAME =d_name) %>% select(ADM2_NAME, inad_diff), by = "ADM2_NAME")
  
  #######
  zn_scat <- zn_women_ml %>% 
    select(inadequate_percent, ADM2_NAME,s_name) %>% 
    inner_join(zinc_target %>% rename(ADM2_NAME =d_name) %>% select(ADM2_NAME, inad_diff), by = "ADM2_NAME")
  
  
  mn_target_scatter <- (vita_target %>% rename(ADM2_NAME =d_name) %>% select(ADM2_NAME, women_inad_perc,men_inad_perc,inad_diff, ADM1) %>% 
                          rename(va_w_inad = women_inad_perc, va_inad_diff = inad_diff, va_m_inad =men_inad_perc )) %>% 
    full_join((folate_target%>% rename(ADM2_NAME =d_name)  %>% select(ADM2_NAME, women_inad_perc,men_inad_perc,inad_diff, ADM1) %>% 
                 rename(fo_w_inad = women_inad_perc, fo_inad_diff = inad_diff, fo_m_inad =men_inad_perc)), by  = c("ADM2_NAME","ADM1")) %>% 
    full_join((iron_target %>% rename(ADM2_NAME =d_name) %>% select(ADM2_NAME, women_inad_perc,men_inad_perc,inad_diff, ADM1) %>% 
                 rename(if_w_inad = women_inad_perc, if_inad_diff = inad_diff, ir_m_inad =men_inad_perc)), by  = c("ADM2_NAME","ADM1"))%>% 
    full_join((zinc_target %>% rename(ADM2_NAME =d_name)  %>%select(ADM2_NAME, women_inad_perc,men_inad_perc,inad_diff, ADM1) %>% 
  ...
  background <- data.frame(lower = c(-50,0), 
                           upper = c(0, 50),
                           col = c("Women more inadequate", "Men more inadequate"))
  ...
  vita_scat_plot <- va_scat %>% 
    ...
    guides(color=guide_legend(title="State"),
           fill=guide_legend(title="Inadeqacy prevalence gap"))+
    coord_fixed()
  ...
  diff_w_scatter <- ggarrange(vita_scat_plot + rremove("ylab") + rremove("xlab"),fol_scat+ rremove("ylab") + rremove("xlab"),
                              ...
                              common.legend = TRUE,
                              labels = c("Vitamin A","Folate", "Iron", "Zinc"),legend="right")
  ...
  diff_w_scatter <-  annotate_figure(diff_w_scatter, left = text_grob("Inadequacy sex difference (percentage)", rot = 90),
                                     ...
                                     top = text_grob("Women more inadequate in folate and iron ",color = "#404080", face = "bold", size = 14))
  
  
  
  ### t test
  
t.test(vita_men_ml$inadequate_percent, vita_women_ml$inadequate_percent)
t.test(fo_men_ml$inadequate_percent, fo_women_ml$inadequate_percent)
t.test(ir_men_ml$inadequate_percent, ir_women_ml$inadequate_percent)
t.test(zn_men_ml$inadequate_percent,zn_women_ml$inadequate_percent)
  
  names(vita_target)
  #overall inadequacy
  
  # Create a data frame for the triangles
  data_triangles <- data.frame(
    x = c(0, 0, 100),
    y = c(0, 100, 100),
    group = c("Top Left", "Bottom Right", "Bottom Right")
  )
  
  # Create the plot
  ggplot(data_triangles, aes(x = x, y = y, group = group, fill = group)) +
    geom_polygon() +
  ...
  va_inad <- vita_men_ml %>% 
    rename(inad_m = inadequate_percent) %>% 
    select(inad_m, d_name,s_name) %>% 
    # filter(s_name != "WEST BENGAL") %>% 
    left_join(vita_women_ml %>% 
    rename(inad_w = inadequate_percent) %>% select(inad_w,d_name),
               by = "d_name") %>% 
    ggplot() +
    geom_point(aes(inad_m, inad_w, color = s_name))+
    # geom_rect(data = background, 
    #           aes(xmin = 0, xmax = 100, ymin = 0, ymax = x, fill = col),
    #           alpha = 0.2, 
    #           show.legend = TRUE) +
    # 
    theme_ipsum() +
  ...
  diff_scatter <- ggarrange(va_inad + rremove("ylab") + rremove("xlab"),fo_inad+ rremove("ylab") + rremove("xlab"),
                              ir_inad+ rremove("ylab") + rremove("xlab"),zn_inad+ rremove("ylab") + rremove("xlab"),
                              common.legend = TRUE,
                              labels = c("Vitamin A","Folate", "Iron", "Zinc"),legend="right")
  
  diff_scatter <-  annotate_figure(diff_scatter, left = text_grob("Percentage inadequacy - women", rot = 90),
                                     bottom = text_grob("Percentage inadequacy - men"),
                                     top = text_grob("Overall micronutrient intake inadequacy", color = "#404080", face = "bold", size = 14))
